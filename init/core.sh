...
echo '========================================================='
mkdir -p ~/testnet-node/kc
cd ~/testnet-node/kc
cardano-cli address key-gen --verification-key-file payment_h1.vkey --signing-key-file payment_h1.skey
chmod 400 payment.skey payment.vkey

cardano-cli stake-address key-gen --verification-key-file stake_o1.vkey --signing-key-file stake_o1.skey
chmod 400 stake.skey stake.vkey

cardano-cli address build \
 --payment-verification-key-file payment_h1.vkey \
 --testnet-magic 1097911063 \
 --out-file payment_h1.addr
...
echo '========================================================='​
UTXO0=$(cardano-cli query utxo --address $(cat payment.addr) --testnet-magic 1097911063 | tail -n 1)
...
cardano-cli transaction build-raw --tx-in $(echo $UTXO0H)#$(echo $UTXO0I) --tx-out $(cat payment.addr)+0 --ttl ${TTL} --fee 0 --certificate stake.cert --out-file stakereg.draft.txraw
...
--tx-in $(echo $UTXO0H)#$(echo $UTXO0I) --tx-out $(cat payment.addr)+$(echo $TXOUT) --ttl $TTL --fee $FEE --certificate-file stake.cert --out-file stakereg.txraw
echo '========================================================='
...
cardano-cli transaction build-raw --tx-in $(echo $UTXO0H)#$(echo $UTXO0I) --tx-out $(cat payment.addr)+0 --ttl ${TTL} --fee 0 --certificate-file pool.cert --certificate-file delegation.cert --out-file dummy.txbody
FEE=$(cardano-cli transaction calculate-min-fee \
